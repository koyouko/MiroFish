...
class OasisProfileGenerator:
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """获取系统提示词"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for opinion simulation, faithfully reproducing real-world context. You MUST return valid JSON; all string values must not contain unescaped newlines. Generate all text content in English."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity below, faithfully reproducing real-world context.
...
2. persona: Detailed persona description (~2000 words, plain text), including:
   ...
   - Social media behavior (posting frequency, content preference, interaction style, language)
   - Viewpoints (stance on topics, what provokes or moves them)
   - Unique traits (catchphrases, special experiences, hobbies)
   - Personal memory (connection to the event and actions/reactions in it)
...
- persona must be a single cohesive paragraph
- Write all content in English (gender must be "male" or "female")
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for the institutional/group entity below, faithfully reproducing real-world context.
...
2. persona: Detailed account profile (~2000 words, plain text), including:
   - Institution basics (official name, nature, founding background, core function)
   - Account positioning (account type, target audience, core purpose)
   - Communication style (language traits, common expressions, avoided topics)
   - Content characteristics (content types, posting frequency, active hours)
   - Stance (official position on key topics, how it handles controversy)
   - Special notes (audience profile, operational habits)
   - Institutional memory (connection to the event and actions/reactions in it)
3. age: Fixed as 30 (virtual age for institutional accounts)
4. gender: Fixed as "other" (institutional accounts are non-personal)
...
- persona must be a single cohesive paragraph with no newline characters
- Write all content in English (gender must be English "other")
- age must be integer 30, gender must be string "other"
- Institutional account tone must match its identity"""
